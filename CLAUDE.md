...
**Scheduled digests:** `scheduler.py` runs as a fourth `asyncio.gather()` task and fires jobs at fixed `Asia/Ho_Chi_Minh` times: `hn.py` (Algolia HN API, keyword-filtered security stories) at `HN_DIGEST_TIMES`, and `voz.py` (voz.vn `f/diem-bao.33` subforum RSS — one thread per curated news article) at `PRESS_DIGEST_TIMES`. Digests are Gemini-composed in `analyzer.py` (`hn_digest`, `press_digest`) and pushed via `bot.send_to_chat` — HN to `OWNER_ID`, the press digest to `PRESS_CHAT_ID` (defaults to `OWNER_ID`, so an unset var keeps the DM behaviour). The press job is the module-level `main.run_press_digest`: a `TelegramAPIError` on the group send warns the owner by DM instead of dying in the logs, because group delivery breaks permanently when the bot is kicked or a basic group migrates to a supergroup and its id changes. `OwnerOnlyMiddleware` is untouched — commands typed in the group by anyone other than `OWNER_ID` are still dropped. Links are inline and code-owned: the LLM writes `(#5)` markers and never a URL, and `analyzer._linkify` HTML-escapes its prose then swaps each marker for an `<a href>` anchor, so digests go out with `parse_mode=HTML` (`bot.NO_PREVIEW` suppresses the auto link card). Everything appended to a digest must pass `analyzer.escape_html` — a raw `<` anywhere in the message makes Telegram reject all of it. `bot._safe_cut` keeps 4096-char chunking from cutting inside an anchor for the same reason. If the model drops every marker, `press_digest` falls back to the old numbered URL block so a digest is never link-less. The press title line is code-owned (`analyzer.PRESS_HEADER`) — the model is told not to write a header or a time of day, which went stale when the push moved to 18:30. voz.vn sits behind Cloudflare, which fingerprints client TLS stacks — `voz.py` fetches through `curl_cffi` impersonating Chrome (sync, wrapped in `asyncio.to_thread`); cloudscraper was tried first and gets 403 from the VM's datacenter IP. If Cloudflare hardens further, the fetch fails soft (empty digest + logged exception) rather than crashing the bot. /paper additionally crawls the forum's pinned news megathreads (sticky block on the forum page; meta stickies like "Nội quy"/"Report" filtered by an anchored title regex) and appends a Vietnamese `megathread_update` section per thread (max 2) via `bot.build_press_report`.
...
| `PRESS_CHAT_ID` | No | `OWNER_ID` | Chat the scheduled press digest goes to (negative id for a group; bot must be a member) |
